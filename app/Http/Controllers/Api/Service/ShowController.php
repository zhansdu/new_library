...
use App\Services\Handlers\GetUserHandler;
use App\Services\Handlers\GetUserReserveListHandler;
...
class ShowController extends Controller
{
    ...
    /**
     * @param Request $request
     * @param GetUserReserveListHandler $handler
     * @return JsonResponse
     */
    public function getUserReserveList(Request $request, GetUserReserveListHandler $handler): JsonResponse
    {
        /** @var User $user */
        $user = User::findOrFail($request->user()->user_cid);

        $reserveList = $handler->handle($user->user_cid);

        return response()->json([
            'res' => $reserveList->toArray(),
        ]);
    }

    /**
     * @param string $userCid
     * @param GetUserHandler $handler
     * @return JsonResponse
     * @throws ReturnResponseException
     */
    public function getUser(string $userCid, GetUserHandler $handler): JsonResponse
    {
